...
在訓練 SSD 模型 200 個 epoch 後，可以驗證性能。 使用了三個可能的評估指標：1) IoU，2) 精確度和 3) 召回率。
...
在本章中，討論了多尺度單鏡頭目標檢測的概念。 使用以感受野塊的質心為中心的錨框，計算地面實況邊界框偏移量。 與原始像素誤差不同，歸一化像素誤差鼓勵更適合優化的有界範圍。


ground truth 類標籤是為每個錨框分配的。 如果錨框不與對象重疊，則為其分配背景類並且不包括其偏移量
在偏移損失計算中。 Focal loss 被提出用來改進類別損失函數。 默認的 L1 offset 損失函數可以替換為平滑的 L1 損失函數。

對測試數據集的評估表明，使用默認損失函數的歸一化偏移可在平均精度和召回率方面取得最佳性能，而當移除偏移歸一化時，mIoU 會得到改善。 可以通過增加訓練圖像的數量和變化來提高性能。
