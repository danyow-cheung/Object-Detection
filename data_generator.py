...
class DataGenerator(Sequence):
    ...
    def __data_generation(self,keys):
        ...
        dim = (self.args.batchs_size,self.n_boxes,4)
        ...
        for i,key in enumerate(keys):
            # image are assumed to be stored in self.args.data_path 
            # key is the image filename
            image_path = os.path.join(self.args.data_paath,key)
            ...
            # 4 bouding box coords are 1st four items of labels
            # last item is object class label
            boxes = labels[:,0:-1]
            for index,feature_shape in enumerate(self.feature_shapes):
                # generate anchor boxes 
                anchors = anchor_boxes(feature_shape,image.shape,index=index,n_layers=self.args.layers)

                # each feature layer has a row of anchor boxes 
                anchors = np.reshape(anchors,[-1,4])

                # compute IoU of each bouding boxes 
                # with respect to each bounding boxes 
                iou = label_utils.iou(anchors,boxes)
